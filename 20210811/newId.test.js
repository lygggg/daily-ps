...
const lowerCase = (id) => {
    return id.split().map(i => i.toLowerCase()).join();
}
...
const doubleWord = (id) => {
    const result = id.split('').filter((value, index) => value !== id.split('')[index+1]).join('');
    return id.replace(/\.+/g, ".");
}
...
test("lowerCase", () => {
    expect(lowerCase("...!@BaT#*..y.abcdefghijklm")).toBe("...!@bat#*..y.abcdefghijklm");
})

test("removeWord", () => {
    expect(removeWord("...!@bat#*..y.abcdefghijklm")).toBe("...bat..y.abcdefghijklm");
})
